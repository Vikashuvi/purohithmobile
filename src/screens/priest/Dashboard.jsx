...
import { CalendarCheck2, CalendarClock, Check, ChevronRight, Clock3, MapPin, MessageSquareText, Phone, ShieldCheck, WalletCards } from "lucide-react-native";
...
export default function PriestDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try { const { data } = await api.get("/bookings/priest"); setItems(data || []); } catch (_) { setItems([]); }
  }, [user?.demo]);
  useEffect(() => { load(); }, [load]);
  ...
  const act = async (booking, action) => {
    ...
    try { await api.post(`/bookings/${booking.id}/action`, { action, reason: action === "reject" ? "Not available" : "" }); load(); }
    catch (error) { Alert.alert("Booking update failed", error?.response?.data?.detail || "Try again."); }
  };
