...
export default function Profile({ navigation }) {
  ...
  const { language, area } = usePreferences();
  const { width } = useWindowDimensions();
  if (!user) return null;
  const priest = user.role === "priest";
  const desktop = width >= 860;
  const rows = [
    { icon: Languages, title: "Language", value: language === "kn" ? "Kannada" : "English" },
    { icon: MapPin, title: "Service location", value: `${area.name}, Bengaluru` },
    { icon: Bell, title: "Notifications", value: "On" },
    { icon: Smartphone, title: "Signed-in devices", value: "1 device" },
    { icon: CircleHelp, title: "Help and support", value: "" },
  ];
  ...
  return <ScrollView style={styles.root} contentContainerStyle={[styles.content, desktop && styles.contentDesktop]} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.settings}>{rows.map((row) => <SettingRow key={row.title} {...row} />)}</View>
    <Pressable onPress={logout} style={({ pressed }) => [styles.logout, pressed && styles.pressed]}><LogOut size={18} color={colors.danger} /><Text style={styles.logoutText}>{t.logout}</Text></Pressable>
...
function SettingRow({ icon: Icon, title, value }) { return <Pressable style={({ pressed }) => [styles.settingRow, pressed && styles.rowPressed]}><Icon size={19} color={colors.ink} /><Text style={styles.settingTitle}>{title}</Text>{value ? <Text style={styles.settingValue}>{value}</Text> : null}<ChevronRight size={18} color="#999995" /></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  name: { color: colors.ink, fontFamily: font.bold, fontSize: 23, lineHeight: 29 }, phone: { color: colors.muted2, fontSize: 12, marginTop: 4 }, member: { color: colors.saffronDark, fontFamily: font.semibold, fontSize: 11, marginTop: 7 }, editButton: { minHeight: 38, justifyContent: "center", paddingHorizontal: 14, borderRadius: 9, borderWidth: 1, borderColor: colors.ink }, editText: { color: colors.ink, fontFamily: font.semibold, fontSize: 12 },
  ...
  practiceBanner: { marginTop: 28, minHeight: 116, padding: 18, borderRadius: 14, backgroundColor: colors.ink, flexDirection: "row", alignItems: "center", gap: 13 }, practiceIcon: { width: 44, height: 44, borderRadius: 10, backgroundColor: colors.saffron, alignItems: "center", justifyContent: "center" }, practiceEyebrow: { color: "#AFAFAC", fontFamily: font.bold, fontSize: 9 }, practiceTitle: { color: colors.white, fontFamily: font.bold, fontSize: 15, marginTop: 5 }, practiceBody: { color: "#BEBEBA", fontSize: 10, lineHeight: 15, marginTop: 4 },
  ...
  logout: { minHeight: 54, marginTop: 24, flexDirection: "row", alignItems: "center", gap: 11, borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.warmBorder }, logoutText: { color: colors.danger, fontFamily: font.semibold, fontSize: 14 }, pressed: { opacity: .7 }, footer: { color: colors.muted2, fontSize: 10, textAlign: "center", marginTop: 24 },
});
