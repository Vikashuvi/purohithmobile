...
import { ArrowRight, House, BookOpen, Sparkles } from "lucide-react-native";
  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.brandRow}>
        <BrandLogo width={210} height={96} showText={false} />
      </View>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroCopy}>
          ...
          <Text style={styles.heroTitle}>Your shubh karya,{"\n"}handled with care.</Text>
        </View>
      </View>
...
function RoleButton({ Icon, title, subtitle, onPress, primary, testID }) {
  return <Pressable testID={testID} onPress={onPress} style={({ pressed }) => [styles.role, primary && styles.rolePrimary, pressed && { opacity: .84 }]}>
    <View style={[styles.roleIcon, primary && styles.roleIconPrimary]}><Icon size={21} color={primary ? colors.white : colors.ink} /></View>
    <View style={{ flex: 1 }}><Text style={[styles.roleTitle, primary && { color: colors.white }]}>{title}</Text><Text numberOfLines={2} style={[styles.roleSub, primary && { color: "#CFCFCB" }]}>{subtitle}</Text></View>
    <ArrowRight size={19} color={primary ? colors.white : colors.ink} />
  </Pressable>;
}

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: colors.white, paddingHorizontal: spacing.xl, paddingTop: 12 },
  brandRow: { minHeight: 96, flexDirection: "row", alignItems: "center", gap: 11 },
  om: { color: colors.white, fontSize: 23 },
  hero: { height: 290, marginTop: 12, borderRadius: radii.xl, overflow: "hidden", backgroundColor: colors.muted },
  heroImage: { width: "100%", height: "100%", resizeMode: "cover" },
  heroShade: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(12,12,10,.32)" },
  heroCopy: { position: "absolute", left: 20, right: 20, bottom: 20 },
  verified: { flexDirection: "row", alignItems: "center", alignSelf: "flex-start", gap: 6, height: 29, paddingHorizontal: 10, borderRadius: 15, backgroundColor: "rgba(20,20,18,.72)" },
  ...
  chooseRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 24, marginBottom: 10 },
  choose: { fontSize: 19, color: colors.ink, fontWeight: "700" },
  step: { fontSize: 11, color: colors.muted2 },
  role: { minHeight: 76, borderWidth: 1, borderColor: colors.warmBorder, borderRadius: radii.lg, padding: 14, flexDirection: "row", alignItems: "center", gap: 13, marginBottom: 10 },
  rolePrimary: { backgroundColor: colors.ink, borderColor: colors.ink },
  roleIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.muted, alignItems: "center", justifyContent: "center" },
  roleIconPrimary: { backgroundColor: "#303030" },
  roleTitle: { color: colors.ink, fontSize: 15, fontWeight: "700" },
  roleSub: { color: colors.muted2, fontSize: 11, lineHeight: 15, marginTop: 3 },
  legal: { color: colors.muted2, fontSize: 9, textAlign: "center", marginTop: "auto", marginBottom: 10 },
});
